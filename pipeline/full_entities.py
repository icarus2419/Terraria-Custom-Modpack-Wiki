...
raw  = json.load(open(os.path.join(BASE, "full_index.json")))
...
for host, s in cand.items():
    t = sorted(s)
    for i in range(0, len(t), 18): jobs.append((host, t[i:i+18]))
...
json.dump({"resolved": resolved,
          ...
          open(os.path.join(BASE, "full_entities.json"), "w"), indent=1)
