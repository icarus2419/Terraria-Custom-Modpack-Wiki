...
.rail{display:flex; align-items:flex-start; overflow-x:auto; padding:11px 0 4px; scrollbar-width:thin}
.rail::-webkit-scrollbar{height:6px}
.rail::-webkit-scrollbar-thumb{background:var(--line-strong); border-radius:3px}
button.step{position:relative; flex:0 0 auto; min-width:86px; display:flex; flex-direction:column;
  align-items:center; gap:5px; padding:2px 4px 8px; background:none; border:0; cursor:pointer;
  font:inherit; color:var(--ink-3)}
button.step::before{content:""; position:absolute; top:19px; left:0; width:50%; height:2px;
  background:var(--line-strong)}
button.step::after{content:""; position:absolute; top:19px; left:50%; width:50%; height:2px;
  background:var(--line-strong)}
...
.step .dot{position:relative; z-index:1; width:38px; height:38px; border-radius:50%; flex:none;
  display:grid; place-items:center; background:var(--slot-bg); border:2px solid var(--line-strong)}
.step .dot img{max-width:22px; max-height:22px; width:auto; height:auto; opacity:.75}
...
/* ---------- class strip ---------- */
.classbar{display:flex; align-items:center; gap:6px; overflow-x:auto; padding:0 0 9px;
...
.pnl > header{display:flex; align-items:baseline; gap:10px; flex-wrap:wrap; padding:10px 15px;
  border-bottom:1px solid var(--line); border-top:3px solid var(--ccol); background:var(--surface-2)}
...
<header class="masthead"><div class="wrap mast-in">
  <div class="brandline"><div>
    ...
    <h1>Loadouts</h1>
    <p class="tagline">One boss, one class, one panel: <b>what to actually equip</b> before the fight.
    Merged from the class-setup guides of Terraria, Thorium, Spirit and The Stars Above onto a single
    timeline &mdash; every other option they list is still here, one click away.</p>
    <ul class="howto">
      <li><b>1</b>Pick the boss you are about to fight</li>
      <li><b>2</b>Pick your class</li>
      <li><b>3</b>Equip what is in the slots</li>
    </ul>
  </div></div>
  <dl class="meta">
    <div><dt>Checkpoints</dt><dd>@@NST@@</dd></div>
...
<main class="wrap lo-main">
  ...
  <div class="keybar">
    <span class="lbl">Slot colour = which wiki recommends it</span>
    ...
    <span class="sep"></span>
    <span class="tag new">New here</span>
    <span class="tag any">Any class</span>
  </div>
...
function scrollTop(){
  var y=document.getElementById("stagehead").getBoundingClientRect().top+window.scrollY
...
function render(){
  ...
  /* --- stage header --- */
  head.textContent="";
  var ic=el("div","bossic");
  if(ICON[cur]){ var im=new Image(); im.src=ICON[cur]; im.alt=""; im.className="px"; ic.appendChild(im); }
  head.appendChild(ic);
  var txt=el("div","txt");
  txt.appendChild(el("p","eyebrow","Checkpoint "+(i+1)+" of "+STAGES.length));
  var h2=el("h2",null,st[1]);
  h2.appendChild(document.createTextNode(" "));
  var era=el("span","era "+(st[3]?"hard":"pre"), st[3]?"hardmode":"pre-hardmode");
  h2.appendChild(era);
  txt.appendChild(h2);
  var line;
  if(i===0) line="A fresh world, nothing killed yet — this is what to open with.";
  else if(i===STAGES.length-1)
    line="Moon Lord is down. Past the end of the vanilla game: "+st[2]+".";
  else line=(i>=2 ? "Beaten: "+STAGES[i-1][2]+". " : "")+"Gearing up for: "+st[2]+".";
  txt.appendChild(el("p",null,line));
  head.appendChild(txt);
  var pg=el("div","progbar"), tr=el("div","track"), fl=el("div","fill");
  fl.style.width=Math.round((i+1)/STAGES.length*100)+"%";
  tr.appendChild(fl); pg.appendChild(tr);
  pg.appendChild(el("p","cap", (i+1)+" / "+STAGES.length+" checkpoints"));
  head.appendChild(pg);
...
/* bring the current checkpoint into view on the rail */
(function(){
  var b=rail.querySelector('button.step[aria-current="true"]');
